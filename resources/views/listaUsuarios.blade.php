    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto">
                @auth
                    <li class="nav-item">
                        <a class="nav-link" href="{{ route('profile') }}"> Usuario: {{ Auth::user()->nombreUsuario }}</a>
                    </li>
                    @if(Auth::user()->admin)
    ...
    <div class="mt-5">
        <table class="table table-striped">
            ...
            <tbody>
                ...
                <tr>
                    <form action="{{ url('/admin/cliente/' . $cliente->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que quieres modificar este cliente?');">
                        @csrf
                        @method('PATCH')
                        <td>{{ $cliente->id }}</td>
                    ...
                    <td>
                        <form action="{{ url('/admin/cliente/' . $cliente->id) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Estás seguro de que quieres eliminar este cliente?');">
                            @csrf
                            @method('DELETE')
